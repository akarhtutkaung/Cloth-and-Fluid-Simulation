...
class Camera
{
  ...
  Camera()
  {
    ...
    // initialized previous mouse position
  ...
  void Update(float dt)
  {
    ...
    if(pmouse.x == mouseX && pmouse.y == mouseY){
      negativeTurn.x = 0;
      positiveTurn.x = 0;
      positiveTurn.y = 0;
      negativeTurn.y = 0;
  ...
  {    
    ...
    Vector2 dir = pmouse.directionTo(mouse);
    
    if(dir.x < 0) negativeTurn.x = 1;
    if(dir.x > 0) positiveTurn.x = -0.5;
    if(dir.y < 0) positiveTurn.y = 0.5;
